...
function origin_customize_register( $wp_customize ) {
	...
	/* 	Gravatar Icon for one selected Category 
		added by FC Stoffel */
	...
	$wp_customize->add_setting( 'fcs_gravatar-category', array(
		'default'        => '',
	) );
	...
	if ( get_theme_mod( 'origin_font_family' ) ) {
		...
		switch ( get_theme_mod( 'origin_font_family' ) ) {			
			...
			case 'Droid Sans':
				...
				break;
			case 'Signika':
				wp_enqueue_style( 'font-signika', 'http://fonts.googleapis.com/css?family=Signika', false, 1.0, 'screen' );
				break;
